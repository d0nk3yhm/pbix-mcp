...
| Color extraction & recolor | **Stable** | `pbix_extract_colors` scans themes + all visuals for hex literals AND ThemeDataColor references (resolved to hex). `pbix_recolor` does global find-and-replace across themes + layout, converts ThemeDataColor refs to direct hex, auto-extends unmapped palette colors, injects per-series/category chart colors, generates themed table alternating row colors, and fixes text-vs-background contrast (WCAG 2.0 luminance). Full palette recolor verified: zero stray colors, readable text on all backgrounds |
