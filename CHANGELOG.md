...
### Fixed
- **Cartesian charts fed a plain numeric column on their value axis now render (were empty).** A column/bar/line/area/pie chart that binds a raw numeric column (not a measure) to `Y` / `Values` / `Y2` / `Size` rendered in Power BI Desktop as title + axes only — no bars/line. Two independent causes, both fixed:
  - **Binding** — `compile_visual_binding` put the value column into the grouping as a second group-by dimension instead of aggregating it. It now mirrors Desktop's implicit aggregation: a bare value-role column is wrapped in `Sum` (numeric — `Int64`/`Double`/`Decimal`) or `CountNonNull` (otherwise), emitted as an `Aggregation` in both the query and `dataTransforms`, with the grouping still listing the value index (Desktop aggregates an `Aggregation` projection, group-bys a plain `Column` one). Explicit `Aggregation` selects are handled too; flat grids (`table`/`tableEx`) and slicers show raw values; measures are untouched.
  - **Model metadata** — the builder wrote `SummarizeBy = 2` (None, "don't summarize") for **every** column, so Power BI refused to aggregate a numeric column on a value axis even with the `Aggregation` node present. Numeric columns (`Int64`/`Double`/`Decimal`) now get `SummarizeBy = 1` (Default → Sum); text / datetime / boolean stay `None`. Matches Desktop's default and the corpus (numeric columns are `Default`).
  Byte-exact to Desktop ground truth (AI Sample `barChart`: `Sum(Int64)` → `Aggregation` Function 0, grouping `[0,1]`, and the source column has `SummarizeBy = Sum`). This fixes the most common OpenBI cartesian chart, which routinely binds `Table.Amount` directly. New `TestValueColumnAggregation` (`tests/test_report_binding.py`) + a builder `SummarizeBy` check.
