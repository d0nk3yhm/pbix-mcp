...
### Fixed
- **Cartesian charts fed a plain numeric column on their value axis now render (were empty).** A column/bar/line/area/pie chart that binds a raw numeric column (not a measure) to `Y` / `Values` / `Y2` / `Size` rendered in Power BI Desktop as title + axes only — no bars/line. `compile_visual_binding` put the value column into the grouping as a second group-by dimension instead of aggregating it, so Desktop had nothing to plot. It now mirrors Desktop's implicit aggregation: a bare value-role column is wrapped in `Sum` (numeric — `Int64`/`Double`/`Decimal`) or `CountNonNull` (otherwise), emitted as an `Aggregation` in both the query and `dataTransforms`, while the grouping still lists the value index (Desktop aggregates an `Aggregation` projection, group-bys a plain `Column` one). Explicit `Aggregation` selects (e.g. an already-summed field) are now handled too. Flat grids (`table`/`tableEx`) and slicers still show raw values; measures are untouched. Byte-exact to Desktop ground truth (AI Sample `barChart` — `Sum(Int64)` → `Aggregation` Function 0, grouping keeps the index). This fixes the most common OpenBI cartesian chart, which routinely binds `Table.Amount` directly. New `TestValueColumnAggregation` in `tests/test_report_binding.py`.
