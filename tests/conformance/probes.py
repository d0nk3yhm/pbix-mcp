...
PROBES: dict[str, list[str]] = {
    ...
    "ROLLUPISSUBTOTAL": ["COUNTROWS(ADDMISSINGITEMS(K[grp], SUMMARIZECOLUMNS(ROLLUPADDISSUBTOTAL(K[grp], \"st\"), \"s\", SUM(F[v])), ROLLUPISSUBTOTAL([st], K[grp])))","COUNTROWS(ADDMISSINGITEMS(K[grp], SUMMARIZECOLUMNS(ROLLUPADDISSUBTOTAL(K[grp], \"st\"), \"s\", SUM(F[v])), ROLLUPISSUBTOTAL(K[grp], [st])))"],
    ...
    # COLUMNSTATISTICS: 20 rows = 15 user columns + one internal RowNumber
    # row per table (verified row-by-row live; INFO.TABLES()=5 proved no auto
    # date/time tables exist in this capture).
    "COLUMNSTATISTICS": ["COUNTROWS(COLUMNSTATISTICS())"],

    "EXTERNALMEASURE": ["EXTERNALMEASURE(\"x\", INTEGER, \"remote\")"],
    "TABLEOF": ["COUNTROWS(TABLEOF([Total V]))"],
    "NATURALJOINUSAGE": ["COUNTROWS(NATURALJOINUSAGE(F))", "NATURALJOINUSAGE(K)"],
    "LOOKUPWITHTOTALS": ["SUMX(F, IF(LOOKUPWITHTOTALS(K[grp], K[k], F[k]) = \"X\", 1, 0))","LOOKUPWITHTOTALS(K[grp], K[k], 2, 0)"],
    ...
    "ROWNUMBER": ["SUMX(F, ROWNUMBER(ORDERBY(F[v])))",
                  "SUMX(F, ROWNUMBER(ORDERBY(F[v])) * F[v])",
                  "SUMX(F, ROWNUMBER(F, ORDERBY(F[v]), PARTITIONBY(F[k])))"],
    "ORDERBY": ["SUMX(F, ROWNUMBER(ORDERBY(F[v], DESC)) * F[v])"],
    "PARTITIONBY": ["SUMX(F, RANK(DENSE, F, ORDERBY(F[v]), PARTITIONBY(F[k])))"],
    "MATCHBY": ["SUMX(F, ROWNUMBER(F, ORDERBY(F[v]), MATCHBY(F[v])))"],
    "RANK": ["SUMX(F, RANK(ORDERBY(F[v])))",
             "SUMX(F, RANK(DENSE, F, ORDERBY(F[k])))",
             "SUMX(F, RANK(SKIP, F, ORDERBY(F[k])))"],
    "INDEX": ["SUMX(INDEX(1, F, ORDERBY(F[v])), F[v])",
              "SUMX(INDEX(-1, F, ORDERBY(F[v])), F[v])"],
    "OFFSET": ["SUMX(F, SUMX(OFFSET(-1, F, ORDERBY(F[v])), F[v]))"],
    "WINDOW": ["SUMX(F, SUMX(WINDOW(1, ABS, 2, ABS, F, ORDERBY(F[v])), F[v]))",
               "SUMX(F, SUMX(WINDOW(-1, REL, 0, REL, F, ORDERBY(F[v])), F[v]))"],
    # ---- visual-calculation family: expected to be Desktop-refused in query
    # context (classification evidence, same family as COLLAPSE/EXPAND)
    "MOVINGAVERAGE": ["MOVINGAVERAGE(SUM(F[v]), 2)"],
    "RUNNINGSUM": ["RUNNINGSUM(SUM(F[v]))"],
    "FIRST": ["FIRST(SUM(F[v]))"],
    "LAST": ["LAST(SUM(F[v]))"],
    "NEXT": ["NEXT(SUM(F[v]))"],
    "PREVIOUS": ["PREVIOUS(SUM(F[v]))"],
    "RANGE": ["RANGE(SUM(F[v]), 1)"],
    "COLLAPSEALL": ["COLLAPSEALL(SUM(F[v]), ROWS)"],
    "EXPANDALL": ["EXPANDALL(SUM(F[v]), ROWS)"],
    # ---- SUMMARIZECOLUMNS adjunct
    # NONVISUAL requires its filter to target a grouped column (Desktop:
    # "kan ikke brukes p\u00e5 kolonner som ikke er grupperte" for a FILTER
    # over non-grouped K[k]).
    "NONVISUAL": ["SUMX(SUMMARIZECOLUMNS(K[grp], NONVISUAL(TREATAS({\"X\"}, K[grp])), \"s\", SUM(F[v])), [s])",
                  "COUNTROWS(SUMMARIZECOLUMNS(K[grp], NONVISUAL(TREATAS({\"X\"}, K[grp])), \"s\", SUM(F[v])))"],
    # ---- INFO.* authorability sweep (raw counts; counts that include auto
    # date/time artifacts get filtered re-probes once authorability is known)
    "INFO.ALTERNATEOFDEFINITIONS": ["COUNTROWS(INFO.ALTERNATEOFDEFINITIONS())"],
    "INFO.ANNOTATIONS": ["COUNTROWS(INFO.ANNOTATIONS()) > 0"],
    "INFO.ATTRIBUTEHIERARCHIES": ["COUNTROWS(INFO.ATTRIBUTEHIERARCHIES())"],
    "INFO.ATTRIBUTEHIERARCHYSTORAGES": ["COUNTROWS(INFO.ATTRIBUTEHIERARCHYSTORAGES())"],
    "INFO.CALCDEPENDENCY": ["COUNTROWS(INFO.CALCDEPENDENCY())"],
    "INFO.CALCULATIONGROUPS": ["COUNTROWS(INFO.CALCULATIONGROUPS())"],
    "INFO.CALCULATIONITEMS": ["COUNTROWS(INFO.CALCULATIONITEMS())"],
    "INFO.CATALOGS": ["COUNTROWS(INFO.CATALOGS())"],
    "INFO.CHANGEDPROPERTIES": ["COUNTROWS(INFO.CHANGEDPROPERTIES())"],
    "INFO.COLUMNPARTITIONSTORAGES": ["COUNTROWS(INFO.COLUMNPARTITIONSTORAGES())"],
    "INFO.COLUMNPERMISSIONS": ["COUNTROWS(INFO.COLUMNPERMISSIONS())"],
    "INFO.COLUMNS": ["COUNTROWS(INFO.COLUMNS())"],
    "INFO.COLUMNSTORAGES": ["COUNTROWS(INFO.COLUMNSTORAGES())"],
    "INFO.CSDLMETADATA": ["COUNTROWS(INFO.CSDLMETADATA())"],
    "INFO.CULTURES": ["COUNTROWS(INFO.CULTURES())"],
    "INFO.DATACOVERAGEDEFINITIONS": ["COUNTROWS(INFO.DATACOVERAGEDEFINITIONS())"],
    "INFO.DATASOURCES": ["COUNTROWS(INFO.DATASOURCES())"],
    "INFO.DELTATABLEMETADATASTORAGES": ["COUNTROWS(INFO.DELTATABLEMETADATASTORAGES())"],
    "INFO.DEPENDENCIES": ["COUNTROWS(INFO.DEPENDENCIES())"],
    "INFO.DETAILROWSDEFINITIONS": ["COUNTROWS(INFO.DETAILROWSDEFINITIONS())"],
    "INFO.DICTIONARYSTORAGES": ["COUNTROWS(INFO.DICTIONARYSTORAGES())"],
    "INFO.EXCLUDEDARTIFACTS": ["COUNTROWS(INFO.EXCLUDEDARTIFACTS())"],
    "INFO.EXPRESSIONS": ["COUNTROWS(INFO.EXPRESSIONS())"],
    "INFO.EXTENDEDPROPERTIES": ["COUNTROWS(INFO.EXTENDEDPROPERTIES())"],
    "INFO.FORMATSTRINGDEFINITIONS": ["COUNTROWS(INFO.FORMATSTRINGDEFINITIONS())"],
    "INFO.FUNCTIONS": ["COUNTROWS(INFO.FUNCTIONS())"],
    "INFO.GENERALSEGMENTMAPSEGMENTMETADATASTORAGES": ["COUNTROWS(INFO.GENERALSEGMENTMAPSEGMENTMETADATASTORAGES())"],
    "INFO.GROUPBYCOLUMNS": ["COUNTROWS(INFO.GROUPBYCOLUMNS())"],
    "INFO.HIERARCHIES": ["COUNTROWS(INFO.HIERARCHIES())"],
    "INFO.HIERARCHYSTORAGES": ["COUNTROWS(INFO.HIERARCHYSTORAGES())"],
    "INFO.KPIS": ["COUNTROWS(INFO.KPIS())"],
    "INFO.LEVELS": ["COUNTROWS(INFO.LEVELS())"],
    "INFO.LINGUISTICMETADATA": ["COUNTROWS(INFO.LINGUISTICMETADATA())"],
    "INFO.MEASURES": ["COUNTROWS(INFO.MEASURES())"],
    "INFO.MODEL": ["COUNTROWS(INFO.MODEL())"],
    "INFO.OBJECTTRANSLATIONS": ["COUNTROWS(INFO.OBJECTTRANSLATIONS())"],
    "INFO.PARQUETFILESTORAGES": ["COUNTROWS(INFO.PARQUETFILESTORAGES())"],
    "INFO.PARTITIONS": ["COUNTROWS(INFO.PARTITIONS())"],
    "INFO.PARTITIONSTORAGES": ["COUNTROWS(INFO.PARTITIONSTORAGES())"],
    "INFO.PERSPECTIVECOLUMNS": ["COUNTROWS(INFO.PERSPECTIVECOLUMNS())"],
    "INFO.PERSPECTIVEHIERARCHIES": ["COUNTROWS(INFO.PERSPECTIVEHIERARCHIES())"],
    "INFO.PERSPECTIVEMEASURES": ["COUNTROWS(INFO.PERSPECTIVEMEASURES())"],
    "INFO.PERSPECTIVES": ["COUNTROWS(INFO.PERSPECTIVES())"],
    "INFO.PERSPECTIVETABLES": ["COUNTROWS(INFO.PERSPECTIVETABLES())"],
    "INFO.PROPERTIES": ["COUNTROWS(INFO.PROPERTIES()) > 0"],
    "INFO.QUERYGROUPS": ["COUNTROWS(INFO.QUERYGROUPS())"],
    "INFO.REFRESHPOLICIES": ["COUNTROWS(INFO.REFRESHPOLICIES())"],
    "INFO.RELATEDCOLUMNDETAILS": ["COUNTROWS(INFO.RELATEDCOLUMNDETAILS())"],
    "INFO.RELATIONSHIPINDEXSTORAGES": ["COUNTROWS(INFO.RELATIONSHIPINDEXSTORAGES())"],
    "INFO.RELATIONSHIPS": ["COUNTROWS(INFO.RELATIONSHIPS())"],
    "INFO.RELATIONSHIPSTORAGES": ["COUNTROWS(INFO.RELATIONSHIPSTORAGES())"],
    "INFO.ROLEMEMBERSHIPS": ["COUNTROWS(INFO.ROLEMEMBERSHIPS())"],
    "INFO.ROLES": ["COUNTROWS(INFO.ROLES())"],
    "INFO.SEGMENTMAPSTORAGES": ["COUNTROWS(INFO.SEGMENTMAPSTORAGES())"],
    "INFO.SEGMENTSTORAGES": ["COUNTROWS(INFO.SEGMENTSTORAGES())"],
    "INFO.STORAGEFILES": ["COUNTROWS(INFO.STORAGEFILES()) > 0"],
    "INFO.STORAGEFOLDERS": ["COUNTROWS(INFO.STORAGEFOLDERS()) > 0"],
    "INFO.STORAGETABLECOLUMNS": ["COUNTROWS(INFO.STORAGETABLECOLUMNS())"],
    "INFO.STORAGETABLECOLUMNSEGMENTS": ["COUNTROWS(INFO.STORAGETABLECOLUMNSEGMENTS()) > 0"],
    "INFO.STORAGETABLES": ["COUNTROWS(INFO.STORAGETABLES())"],
    "INFO.TABLEPERMISSIONS": ["COUNTROWS(INFO.TABLEPERMISSIONS())"],
    "INFO.TABLES": ["COUNTROWS(INFO.TABLES())"],
    "INFO.TABLESTORAGES": ["COUNTROWS(INFO.TABLESTORAGES())"],
    "INFO.USERDEFINEDFUNCTIONS": ["COUNTROWS(INFO.USERDEFINEDFUNCTIONS())"],
    "INFO.VARIATIONS": ["COUNTROWS(INFO.VARIATIONS())"],
    "INFO.VIEW.COLUMNS": ["COUNTROWS(INFO.VIEW.COLUMNS())"],
    "INFO.VIEW.MEASURES": ["COUNTROWS(INFO.VIEW.MEASURES())"],
    "INFO.VIEW.RELATIONSHIPS": ["COUNTROWS(INFO.VIEW.RELATIONSHIPS())"],
    "INFO.VIEW.TABLES": ["COUNTROWS(INFO.VIEW.TABLES())"],
}
